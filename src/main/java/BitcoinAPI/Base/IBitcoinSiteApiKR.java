...
public abstract class IBitcoinSiteApiKR extends IBitcoinSiteApi
{
    ...
    protected double multiplyAsReigon(EnumReigonType reigon, double price)
    {
        try
        {
            switch(reigon)
            {
            case KRW: return price;
            case USD: return BitCoinApiUtils.KrwToUsd(price);
            case CNY: return BitCoinApiUtils.KrwToCny(price);
            }
        } catch(Exception e) {
