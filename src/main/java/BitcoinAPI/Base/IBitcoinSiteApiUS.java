...
public abstract class IBitcoinSiteApiUS extends IBitcoinSiteApi
{
    ...
    protected double multiplyAsReigon(EnumReigonType reigon, double price)
    {
        try
        {
            switch(reigon)
            {
            case KRW: return BitCoinApiUtils.UsdToKrw(price);
            case USD: return price;
            case CNY: return BitCoinApiUtils.UsdToCny(price);
            }
        } catch(Exception e) {
